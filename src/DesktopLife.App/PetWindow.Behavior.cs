...
public partial class PetWindow
{
    ...
    public BodyAction PhysiologicalAction=>sequence?.Kind==SequenceKind.Sleep&&sequence.Phase!=BehaviorPhase.Sleep?BodyAction.Sit:body.Action;
    ...
    {
        sequence=null;attentionPoint=null;phaseContact=false;activeCare=requestedCare;requestedCare=null;
        ...
        if(action==BodyAction.Sleep)
        {
            ...
            var spots=Furniture.SelectMany(f=>f.Platforms.Where(p=>Math.Abs(p.Y-p.EndY)<1).Select((p,i)=>new RestSpot(f.Item.Id+":"+i,f.Item.Kind,p.X+p.Width/2,p.Y))).ToList();
            ...
            restSpot=restPreference.Choose(spots.Where(s=>RoomNavigation.Plan(platforms,bounds,body.X+58,body.Y+144,s.X,s.Feet) is not null),BehaviorPersonality,Bond,body.X+58,cursor?.X);
            sequence=new(SequenceKind.Sleep,BehaviorPersonality,Bond,restSpot?.Id);return;
        }
    ...
    private void EndSequence()
    {
        ...
        sequencePoseAge=null;activeCare=null;
    }
    ...
    private bool TickSequence(double dt)
    {
        ...
        poseAction=s.Phase==BehaviorPhase.Approach?BodyAction.Walk:BodyAction.ObserveCursor;
    ...
    {
        var exists=restSpot is null||restSpot.Kind is null||Furniture.Any(f=>restSpot.Id.StartsWith(f.Item.Id.ToString(),StringComparison.Ordinal));
        ...
        if(before!=s.Phase&&s.Phase==BehaviorPhase.Sleep&&restSpot is {} used)restPreference.Used(used.Id);
        if(s.TargetId is null)restSpot=null;
        ...
        sequencePoseAge=s.Phase switch{BehaviorPhase.LieDown=>5,BehaviorPhase.Curl or BehaviorPhase.Sleep=>25+s.PhaseAge%15,BehaviorPhase.Stretch=>s.PhaseAge,_=>s.PhaseAge};
